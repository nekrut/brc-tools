...
Both are pure functions of the two collections' element identifiers, so nothing
has to be hand-authored per panel.
"""
import argparse
...
def main(argv=None):
    ...
    with open(args.keep, "w") as kf, open(args.relabel, "w") as rf:
        for a, q in pairs:
            kf.write(f"{a}.{q}\n")

...
> 🗺️ **[Pipeline documentation →](https://nekrut.github.io/brc-tools/)**
>
> The front page maps every workflow (A→K) and shows which output of each feeds
> which input of the next. Hover a workflow to trace its links; solid edges are
> wired, dashed ones name the intended source but still need reshaping by hand.
>
> Cards marked **✓ detail** have been reviewed and link to a step-level page
> where every node carries a real sample from a named invocation. Cards marked
> **unreviewed** are drawn from their workflow files alone — nothing has been
> captured from them yet. Six of twelve are reviewed: A, B, C, C2, E, I.
>
> Both pages are generated, never hand-edited
> ([`gen_pipeline_dataflow.py`](workflows/gen_pipeline_dataflow.py),
> [`gen_pipeline_io_map.py`](workflows/gen_pipeline_io_map.py)) and the overview
> is regenerated by CI on every workflow change.
